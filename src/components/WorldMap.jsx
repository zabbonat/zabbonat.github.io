...
const WorldMap = ({ onNavigate }) => {
    ...
    // Dialog State
    const [showDialog, setShowDialog] = useState(() => !sessionStorage.getItem('hasSeenWelcome_v5'));
    ...
    const advanceDialog = () => {
        if (dialogStep < dialogLines.length - 1) {
            ...
            setShowDialog(false);
            sessionStorage.setItem('hasSeenWelcome_v5', 'true');
        }
    };
                        ...
                        <AnimatePresence>
                            {showDialog && (
                                <motion.div
                                    ...
                                    className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 -ml-7 w-64 bg-white border-4 border-black p-4 rounded-lg shadow-xl z-50 pointer-events-auto cursor-pointer"
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        advanceDialog();
                                    }}
