...
const WorldMap = ({ onNavigate }) => {
  ...
  // 5 Zones for the new Landscape Map
  // Layout: 
  // Top Left: Tavern (About)
  // Top Right: Lab (Research)
  // Bottom Left: Library (Publications)
  // Bottom Right: Tower (CV)
  // Center: School (Teaching)
  const zones = [
    { id: 'about', name: 'The Tavern', desc: '(About Me)', x: 15, y: 25, radius: 15 },
    { id: 'research', name: 'Alchemist Lab', desc: '(Research)', x: 85, y: 25, radius: 15 },
    { id: 'publications', name: 'The Library', desc: '(Publications)', x: 15, y: 75, radius: 15 },
    { id: 'cv', name: 'Archives', desc: '(CV & Resume)', x: 85, y: 75, radius: 15 },
    { id: 'teaching', name: 'The Academy', desc: '(Teaching)', x: 50, y: 50, radius: 15 },
  ];
  ...
  useEffect(() => {
    ...
    zones.forEach((zone) => {
      ...
      // Adjust distance calculation for aspect ratio roughly
      // Since map is wide, X distance is "worth" less in % than Y distance visually, but simple radius is fine
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden flex items-center justify-center select-none">
      {/* Map Background - Cover to fill screen immersively */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: "url('/world_map_landscape.png')" }}
      />
      ...
      >
        ...
        <div className="bg-black/50 px-3 py-1 rounded-full mt-[-10px]">
          <span className="font-pixel text-[10px] text-white">You (Lvl 31)</span>
        </div>
      </motion.div>
    </div>
  );
